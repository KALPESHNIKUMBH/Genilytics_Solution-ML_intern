...
lasso_model <- cv.glmnet(x = as.matrix(train_data[, -ncol(train_data)]), 
                         y = as.numeric(train_data$`default payment next month`), 
                         alpha = 1, family = "binomial")

# Make predictions on the training data
train_predictions <- predict(lasso_model, s = lasso_model$lambda.min, 
...
test_predictions <- predict(lasso_model, s = lasso_model$lambda.min, 
